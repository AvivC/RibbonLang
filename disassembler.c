...
int disassembler_do_single_instruction(OP_CODE opcode, Bytecode* chunk, int offset) {
	...
	switch (opcode) {
		...
		case OP_DECLARE_EXTERNAL: {
			return constant_instruction("OP_DECLARE_EXTERNAL", chunk, offset);
		}
		...
		case OP_DUP_TWO: {
			return simple_instruction("OP_DUP_TWO", chunk, offset);
		}
		case OP_SWAP: {
			return simple_instruction("OP_SWAP", chunk, offset);
		}
		case OP_SWAP_TOP_WITH_NEXT_TWO: {
			return simple_instruction("OP_SWAP_TOP_WITH_NEXT_TWO", chunk, offset);
		}
