...
static void my_thing_a_dealloc(ObjectInstance* instance) {
    ...
    ribbon.deallocate(my_thing_a->dynamic_memory, strlen(my_thing_a->dynamic_memory) + 1, ribbon.EXTENSION_ALLOC_STRING_CSTRING);
}
...
static bool my_thing_a_init(Object* self, ValueArray args, Value* out) {
    ...
    a->dynamic_memory = ribbon.copy_cstring(text->chars, text->length, ribbon.EXTENSION_ALLOC_STRING_CSTRING);
    a->x = 0;
    *out = MAKE_VALUE_NIL();
    return true;
}
...
static bool my_thing_a_get_text(Object* self, ValueArray args, Value* out) {
    ObjectInstanceMyThingA* a = (ObjectInstanceMyThingA*) self;
    *out = MAKE_VALUE_OBJECT(ribbon.object_string_copy_from_null_terminated(a->dynamic_memory));
    return true;
}
...
static bool my_thing_b_get_text_multiplied(Object* self, ValueArray args, Value* out) {
    ...
    ribbon.value_array_init(&get_text_args);
    Value text_out;
    if (!my_thing_a_get_text((Object*) b->a, get_text_args, &text_out)) {
        *out = MAKE_VALUE_NIL();
        return false;
    }
    ribbon.value_array_free(&get_text_args);

    ObjectString* text = (ObjectString*) text_out.as.object;

    char* buffer = ribbon.allocate(text->length * times + 1, ribbon.EXTENSION_ALLOC_STRING_CSTRING);
    ...
    ObjectString* twice = ribbon.object_string_take(buffer, strlen(buffer));
...
static bool my_thing_a_descriptor_get(Object* self, ValueArray args, Value* out) {
    ...
    assert(args.count == 2);
    assert(ribbon.is_value_instance_of_class(args.values[0], "MyThingA"));
    ...
    assert(ribbon.cstrings_equal(attr_name->chars, attr_name->length, "x", 1));
...
static bool square(Object* self, ValueArray args, Value* out) {
    ...
    Value throwaway;
    bool we_have_multiply_also = ribbon.object_load_attribute_cstring_key((Object*) this, "multiply", &throwaway);
    ...
    ObjectModule* other_extension = ribbon.vm_get_module_cstring("myuserextension");
    if (other_extension == NULL) {
        printf("Other extension not imported yet. Importing it.\n");
        if (ribbon.vm_import_module_cstring("myuserextension") == IMPORT_RESULT_SUCCESS) {
        ...
        other_extension = ribbon.vm_get_module_cstring("myuserextension");
    ...
    if (!ribbon.object_load_attribute_cstring_key((Object*) other_extension, "multiply", &multiply_method_val)) {
    ...
    ribbon.value_array_init(&multiply_args);
    ribbon.value_array_write(&multiply_args, &number);
    ribbon.value_array_write(&multiply_args, &number);
    Value sqr_result;
    ribbon.vm_call_function((ObjectFunction*) multiply_method_val.as.object, multiply_args, &sqr_result);
    ribbon.value_array_free(&multiply_args);
...
static bool more_talk_with_other_extension(Object* self, ValueArray args, Value* out) {
    ribbon.vm_import_module_cstring("myuserextension");
    ObjectModule* other_extension = ribbon.vm_get_module_cstring("myuserextension");
    ...
    Value mythinga_val;
    if (!ribbon.object_load_attribute_cstring_key((Object*) other_extension, "MyThingA", &mythinga_val)) {
        FAIL("myuserextension doesn't have a MyThingA attribute.");
    }

    if (!ribbon.object_value_is(mythinga_val, OBJECT_CLASS)) {
    ...
    ribbon.value_array_init(&mythinga_args);
    ribbon.value_array_write(&mythinga_args, &MAKE_VALUE_OBJECT(ribbon.object_string_copy_from_null_terminated("Hyelloooo!")));
    Value instance_val;
    ribbon.vm_instantiate_class(my_thing_a, mythinga_args, &instance_val);
    ribbon.value_array_free(&mythinga_args);

    assert(ribbon.object_value_is(instance_val, OBJECT_INSTANCE));

    ObjectInstance* instance = (ObjectInstance*) instance_val.as.object;

    assert(ribbon.cstrings_equal(instance->klass->name, strlen(instance->klass->name), "MyThingA", strlen("MyThingA")));

    Value get_text_val;
    ribbon.object_load_attribute_cstring_key((Object*) instance, "get_text", &get_text_val);

    assert(ribbon.object_value_is(get_text_val, OBJECT_BOUND_METHOD));
    ...
    ribbon.vm_call_bound_method(get_text, get_text_args, &get_text_result); 
    ribbon.value_array_free(&get_text_args);
    ...
    char* buffer = ribbon.allocate(string_length + 1, ribbon.EXTENSION_ALLOC_STRING_CSTRING);
    snprintf(buffer, string_length + 1, "Text received from MyThingA::get_text(): %.*s", text->length, text->chars);
    ObjectString* result_text = ribbon.object_string_take(buffer, string_length);

    ribbon.vm_push_object((Object*) result_text);

    ValueArray multiply_args;
    ribbon.value_array_init(&multiply_args);
    ribbon.value_array_write(&multiply_args, &MAKE_VALUE_NUMBER(100));
    ribbon.value_array_write(&multiply_args, &MAKE_VALUE_NUMBER(3.5));
    
    Value multiply_result;
    CallResult multiply_call_result = ribbon.vm_call_attribute_cstring((Object*) other_extension, "multiply", multiply_args, &multiply_result);
    ...
    ObjectModule* math_module = ribbon.vm_get_module_cstring("math"); // The stdlib module written in ribbon
    if (math_module == NULL) {
        if (ribbon.vm_import_module_cstring("math") != IMPORT_RESULT_SUCCESS) {
            FAIL("Importing module math failed.");
        }
        
        math_module = ribbon.vm_get_module_cstring("math");
    ...
    ValueArray max_args;
    ribbon.value_array_init(&max_args);
    ribbon.value_array_write(&max_args, &MAKE_VALUE_NUMBER(8));
    ribbon.value_array_write(&max_args, &MAKE_VALUE_NUMBER(5));
    Value max_out;
    ribbon.vm_call_attribute_cstring((Object*) math_module, "max", max_args, &max_out);
    ribbon.value_array_free(&max_args);

    ObjectTable* result_table = ribbon.object_table_new_empty();
    ribbon.table_set(&result_table->table, 
            MAKE_VALUE_OBJECT(ribbon.object_string_copy_from_null_terminated("Text")), MAKE_VALUE_OBJECT(result_text));
    ribbon.table_set(&result_table->table, 
            MAKE_VALUE_OBJECT(ribbon.object_string_copy_from_null_terminated("Number")), multiply_result);
    ribbon.table_set(&result_table->table, 
            MAKE_VALUE_OBJECT(ribbon.object_string_copy_from_null_terminated("MaxResult")), max_out);

    *out = MAKE_VALUE_OBJECT(result_table);

    ribbon.vm_pop_object();
...
MYEXTENSIONAPI bool ribbon_module_init(RibbonApi api, ObjectModule* module) {
    ...
    ObjectFunction* constructor_a = ribbon.make_native_function_with_params("@init", 1, (char*[]) {"text"}, my_thing_a_init);

    ObjectInstance* my_thing_a_descriptor = ribbon.object_descriptor_new_native(my_thing_a_descriptor_get, my_thing_a_descriptor_set);
    ObjectClass* my_thing_a_class = ribbon.object_class_native_new(
            "MyThingA", sizeof(ObjectInstanceMyThingA), my_thing_a_dealloc, NULL, constructor_a,
            (void*[][2]) {{"x", my_thing_a_descriptor}, {NULL, NULL}});

    ribbon.object_set_attribute_cstring_key((Object*) my_thing_a_class, "get_text", MAKE_VALUE_OBJECT(
        ribbon.make_native_function_with_params("get_text", 0, NULL, my_thing_a_get_text)
    ));
    
    ribbon.object_set_attribute_cstring_key((Object*) module, "MyThingA", MAKE_VALUE_OBJECT(my_thing_a_class));
    #endif
    ...
    ObjectFunction* constructor_b = ribbon.make_native_function_with_params("@init", 1, (char*[]) {"a"}, my_thing_b_init);
    ObjectClass* my_thing_b_class 
            = ribbon.object_class_native_new("MyThingB", sizeof(ObjectInstanceMyThingB), my_thing_b_dealloc, my_thing_b_gc_mark, constructor_b, NULL);

    ribbon.object_set_attribute_cstring_key((Object*) my_thing_b_class, "get_text_multiplied", MAKE_VALUE_OBJECT(
        ribbon.make_native_function_with_params("get_text_multiplied", 1, (char*[]) {"times"}, my_thing_b_get_text_multiplied)
    ));

    ribbon.object_set_attribute_cstring_key((Object*) module, "MyThingB", MAKE_VALUE_OBJECT(my_thing_b_class));
    #endif
