...
static void process_string_literal(const char* string, int length, CharacterArray* char_array) {
...
static AstNode* string(int expression_level) {
    CharacterArray processed;
    character_array_init(&processed);

    do {
        const char* string_start = parser.previous.start + 1;
        int string_length = parser.previous.length - 2;

        process_string_literal(string_start, string_length, &processed);
    } while (match(TOKEN_STRING) || match_after_newlines(TOKEN_STRING));

    return (AstNode*) ast_new_node_string(processed);
    // return (AstNode*) ast_new_node_string(buffer, string_length);
}
